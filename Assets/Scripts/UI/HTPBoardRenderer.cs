...
public class HTPBoardRenderer : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        howToPlayInput = gameObject.GetComponent<HowToPlayInput>();
    ...
    public IEnumerator MouseDragFeedback (Vector2Int clickCell, Board board)
    {
        ...
        while (!Mouse.current.leftButton.wasReleasedThisFrame)
        {
            ...
            Vector2 mousePos = howToPlayInput.GetCurrentMousePos();
            mouseProjPos = howToPlayInput.OrthoProjectionFromOrigin(mousePos,startPos);
            Vector2Int mouseProjCell = (Vector2Int)grid.LocalToCell(mouseProjPos);
            mouseProjCell.y *= -1;

            Vector2Int d = howToPlayInput.PointsToDirection(startPos,mouseProjPos);
            d.y *= -1;
